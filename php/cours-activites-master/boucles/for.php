...
foreach ($college as $class => $info) {
  echo '<h3>'.$class.'</h3><br>';
  foreach ($info as $label => $contenu) {
    echo $label.'<br>';
    foreach ($contenu as $nom => $prenom) {
      echo '<strong>'.$nom.'</strong> : '.$prenom .'<br>';
...
foreach ($videotheque as $cleFilm => $info) {
  echo '<h3>Film '.$cleFilm.'</h3><br>';
  foreach ($info as $label => $contenu) {
    echo '<strong>'.strtoupper($label).'</strong> : ';
    echo $contenu.'<br>';
    foreach ($contenu as $nom => $date) {
      echo $date .'<br>';
